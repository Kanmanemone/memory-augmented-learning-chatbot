...
- **(추가 발견) 컨텍스트 구성 파이프라인 자체가 데모 시나리오에 튜닝돼 있음**: 위 5개 가드보다 범위가 넓다. `extract_referent_candidates`의 우선순위 로직(`_candidate_topic_matches_context`, `_referent_candidate_specificity`, `chatbot.py:546-564`)이 `programming:python-decorators`/`programming:python` 토픽 태그를 코드에 직접 명시해 특별 취급하고, `is_context_recovery_utterance`(`chatbot.py:704-708`)가 반응하는 "모호한 발화" 트리거는 사실 `CONTEXT_RECOVERY_UTTERANCES`(`chatbot.py:250-255`)라는 정확히 두 개의 한국어 문자열뿐이다. 이 둘에 의존하는 `build_stm_insufficiency_trigger/context`(`chatbot.py:735-783`)도 마찬가지로 그 두 문자열 밖에서는 작동하지 않는다. `memory/demo_fixture.py`의 `DEMO_LEARNER_PERSONA`/`DEMO_STM_CONTEXT`/`DEMO_LTM_SUMMARY`가 이 파이프라인이 정확히 맞아떨어지도록 설계된 고정 시나리오(데코레이터/wrapper/재귀를 배우는 초급 학습자)를 코드로 명시하고 있어, "referent 복구"라는 이름의 일반 기능처럼 보이는 이 파이프라인 전체가 실제로는 그 한 시나리오를 재현하기 위한 것임이 확인된다. 상세 표는 `docs/ARCHITECTURE.md`의 "컨텍스트 구성 파이프라인" 참고.
